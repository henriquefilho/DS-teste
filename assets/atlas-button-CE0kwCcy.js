class r extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"}),this._isRendering=!1}static get observedAttributes(){return["label","variant","color","size","icon","icon-name","disabled","loading","full-width"]}connectedCallback(){this.render()}attributeChangedCallback(t,o,a){this._isRendering||o===a||this.shadowRoot&&this.render()}get label(){return this.getAttribute("label")||this.textContent.trim()||"Button"}get variant(){return this.getAttribute("variant")||"filled"}get color(){return this.getAttribute("color")||"primary"}get size(){return this.getAttribute("size")||"md"}get icon(){return this.hasAttribute("icon")}get iconName(){return this.getAttribute("icon-name")||"plus"}get disabled(){return this.hasAttribute("disabled")}get loading(){return this.hasAttribute("loading")}get fullWidth(){return this.hasAttribute("full-width")}setupEventListeners(){const t=this.shadowRoot.querySelector("button");t&&t.addEventListener("click",o=>{if(this.disabled||this.loading){o.preventDefault(),o.stopPropagation();return}})}render(){if(this._isRendering)return;this._isRendering=!0;const t=this.disabled||this.loading,o=this.loading?`<atlas-icon name="loader" size="${this.size==="xl"?"md":"sm"}" class="btn__spinner"></atlas-icon>`:this.icon?`<atlas-icon name="${this.iconName}" size="${this.size==="xl"?"md":"sm"}" class="btn__icon"></atlas-icon>`:"";this.shadowRoot.innerHTML=`
      <style>
        :host {
    ...
    `,this._isRendering=!1,this.setupEventListeners()}}customElements.define("atlas-button",r);
